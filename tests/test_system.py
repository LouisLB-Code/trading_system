...
class TestRiskManagement:
    ...
    def test_risk_limits(self, setup):
        ...
        # Prepare test orders
        orders = self._prepare_test_orders()
        
        # Validate orders
        valid_orders = [
            order for order in orders
            if self.strategy._validate_order(order)
        ]
        
        assert len(valid_orders) <= len(orders)
        for order in valid_orders:
            assert order['size'] * order['entry_price'] <= \
                   self.config.MAX_POSITION_SIZE * 10000

def run_tests():
    """Run all tests"""
    pytest.main(['-v', 'tests/test_system.py'])
